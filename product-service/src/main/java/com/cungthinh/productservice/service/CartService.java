...
public class CartService {
    ...
    public CartService(CartRepository cartRepository, MongoTemplate mongoTemplate, ProductRepository productRepository, CartMapper cartMapper, CartProductRepository cartProductRepository) {
    ...
    public void updateCartProductQuantity(String userId, String productId, int quantity) {
        ...
        query.addCriteria(Criteria.where("userId").is(userId).and("cartProducts.productId").is(productId));
    ...
    public void deleteCartProduct(String userId, String productId) {
        ...
        query.addCriteria(Criteria.where("userId").is(userId)
                .and("cartProducts.productId").is(productId)
                .and("cartProductCount").gt(0));
        Update update = new Update();
    ...
    public void addToCart(CartCreationRequest request) {
        Product foundProduct = productRepository.findById(request.getProductId()).orElseThrow();
    ...
    public void updateCart(CartUpdateRequest request) {
        ...
        updateCartProductQuantity(request.getUserId(), request.getProductId(),
                request.getQuantity() - request.getOldQuantity());
    }
