...
public class InventoryService {
    ...
    public InventoryService(InventoryRepository inventoryRepository, InventoryReservationRepository inventoryReservationRepository, MongoTemplate mongoTemplate) {
    ...
    public boolean reserveInventory(String productId, int quantity, String cartId) {
        Query query = new Query(Criteria.where("productId").is(productId).and("stock").gte(quantity));
        Update update = new Update();
        update.inc("stock", -quantity);

        InventoryReservation inventoryReservation = InventoryReservation.builder().cartId(cartId).quantity(quantity).build();
    ...
    public boolean isStockAvailable(String productId, int quantity) {
        Inventory inventory = inventoryRepository.findByProductId(productId).orElseThrow(() -> new CustomException(ErrorCode.INVENTORY_NOT_FOUND));
